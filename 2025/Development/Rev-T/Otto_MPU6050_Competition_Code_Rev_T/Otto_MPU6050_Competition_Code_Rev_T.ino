...
void takeStepWithCorrection() {
  ...
  if (abs(heading_error) < 2.5) {
    ...
    // SWAPPED LEFT/RIGHT since Otto's directions are backwards
    if (heading > target_heading + 2.5) {
      // Otto drifted RIGHT, need to correct back to center
      Serial.print(F("DRIFT_RIGHT: correct with Otto.turn(RIGHT) H="));
      Serial.println(heading, 2);
      Otto.turn(1, CORRECTION_DELAY, RIGHT); // SWAPPED
      delay(150);
      Otto.walk(1, current_speed, FORWARD);
      
    } else if (heading < target_heading - 2.5) {
      // Otto drifted LEFT, need to correct back to center
      Serial.print(F("DRIFT_LEFT: correct with Otto.turn(LEFT) H="));
      Serial.println(heading, 2);
      Otto.turn(1, CORRECTION_DELAY, LEFT); // SWAPPED
      delay(150);
      Otto.walk(1, current_speed, FORWARD);
    }
  }
